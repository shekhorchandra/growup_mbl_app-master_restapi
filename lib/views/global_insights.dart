...
class GlobalInsightsSlider extends StatefulWidget {
  ...
  @override
  State<GlobalInsightsSlider> createState() => _GlobalInsightsSliderState();
}

class _GlobalInsightsSliderState extends State<GlobalInsightsSlider> {
  ...
  Future<void> fetchGlobalInsights() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          globalInsights = jsonData['globalInsights'] ?? [];
          isLoading = false;
        });
      } else {
      ...
      debugPrint('Error fetching global insights: $e');
      setState(() => isLoading = false);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (globalInsights.isEmpty) {
      return const Center(child: Text('No global insights available'));
    }
    ...
    return Column(
      ...
      children: [
        CarouselSlider.builder(
          itemCount: globalInsights.length,
          itemBuilder: (context, index, realIndex) {
            final item = globalInsights[index];
                    ...
                    child: IconButton(
                      ...
                      onPressed: () {
                        Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (_) =>
                                FullscreenVideoPlayer(url: mediaPath),
                          ),
                        );
                      },
                    ),
        ...
        Row(
          ...
          children: globalInsights.asMap().entries.map((entry) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text("Global Insight",
